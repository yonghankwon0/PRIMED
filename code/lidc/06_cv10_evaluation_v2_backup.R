...
csv_path <- "./output_v2/radiomics_cd.csv"
...
cat(sprintf("PRIMED lambda: data-driven log-spaced 50 values\n"))
...
for (fold in 1:n_folds) {
  ...
  cat("  PRIMED: ")
  t0 <- proc.time()
  pr <- primed_cv_fit(X_bar_tr, S_tr, y_tr, n_lambda=50,
...
cat(sprintf("\nPRIMED lambda: mean=%.4f (%.4f), mean idx=%.1f/%d\n",
    ...
    mean(res_df$primed_lambda_idx), 50))
...
write.csv(res_df, "./output_v2/cv10v2_auc_results.csv", row.names=FALSE)
write.csv(sel_summary, "./output_v2/cv10v2_selection_frequency.csv", row.names=FALSE)
saveRDS(list(results=res_df, selected_vars=selected_vars, sel_summary=sel_summary),
        "./output_v2/cv10v2_full_results.rds")
