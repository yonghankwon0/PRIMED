...
%%%% Standard Packages
% === Modification marker (remove before final submission) ===
...
\keywords{Multi-reader imaging, Variable selection, Group LASSO, Inter-reader disagreement, Consensus--disagreement framework, Rectal cancer, \modified{Radiomics, LIDC-IDRI, Lung nodule}}
...
\]
\modified{If the number of readers varies across subjects (e.g., due to missing evaluations or, as in the LIDC-IDRI application, varying numbers of annotators per nodule), $m$ is replaced by the subject-specific count $m_i$ in the definitions above.}
...
\modified{The group structure ensures that each feature is evaluated holistically (considering both its consensus signal ($\beta_k$) and its residual disagreement signal ($\gamma_k$)), rather than allowing one channel to be selected in isolation, which could lead to the inclusion of features with a spurious consensus or disagreement effect.
Since each group consistently contains two coefficients, group-specific weights (such as $\sqrt{p_g}$ scaling) are unnecessary and are omitted.}

The intercepts $\alpha_{0k}$ and $\beta_0$ are unpenalized, as is standard.
Crucially, the dispersion slopes $\alpha_k$ are also unpenalized: they are updated by gradient descent without \modified{penalty-induced} shrinkage.
...
This selective penalization ensures that the consensus--disagreement relationship is estimated without \modified{penalty-induced} bias regardless of outcome relevance, while variable selection is driven solely by the outcome coefficients $(\beta_k, \gamma_k)$.

\modified{All predictors ($\bar X_k$ and $S_k$) are standardized to zero mean and unit variance within the training set prior to penalization, ensuring that the group LASSO penalty is applied on a common scale across features with different units.}
...
The dispersion coefficients $\alpha_k$ are never shrunk, ensuring that the consensus--disagreement relationship is estimated without \modified{penalty-induced} bias regardless of outcome relevance.
...
\modified{A practical concern with non-convex objectives is that the regularization path may be non-monotone: decreasing $\lambda$ could, in principle, cause previously selected features to drop out, because the change in $\hat\alpha_k$ alters $\tilde S_{ik}$ and thereby reshapes the loss landscape.
...
\end{equation}
\modified{This decomposition is structurally similar in form to the total/direct/indirect effect partition used in mediation analysis, but it is applied here to the linear predictor of a penalized regression and does not carry a causal interpretation.
...
\modified{The regularization parameter $\lambda$ controls the sparsity of the solution: larger $\lambda$ sets more $(\beta_k, \gamma_k)$ pairs to zero.
...
%%-------------------------------------------------------------------
\modified{\subsection{Nonlinear dispersion model}\label{sec:nonlinear}}
%%-------------------------------------------------------------------

\modified{The linear dispersion model $S_{ik} = \alpha_{0k} + \alpha_k \bar X_{ik} + \varepsilon_{ik}$ is the simplest choice, but the consensus--disagreement relationship need not be linear.
...
\begin{equation}\label{eq:quad_disp}
  \modified{S_{ik} = \alpha_{0k} + \alpha_{1k} \bar X_{ik} + \alpha_{2k} \bar X_{ik}^2 + \varepsilon_{ik}}
\end{equation}
\modified{captures the inverted-U pattern with a single additional parameter per feature.
...
\[
  \modified{\frac{\partial \mathcal{L}_{\mathrm{out}}}{\partial \alpha_{1k}}
...
\]
\modified{All other components of the framework (the outcome model, the group penalty on $(\beta_k, \gamma_k)$, and the proximal gradient algorithm) remain unchanged.
...
Using the same $K = 20$ structure as Section~\ref{sec:noise_sim}, we vary $\tau \in \{0.3, 0.6, 0.9\}$ \modified{under all four DGP$\times$scenario combinations}.
...
\modified{Table~\ref{tab:high_tau_full} extends this analysis to all four DGP$\times$scenario combinations.
...
\modified{We increase the number of features to $K = 50$ and $K = 100$, with $n = 200$, under both DGP~1 and DGP~2.}
All other settings match Section~\ref{sec:noise_sim}: $\tau = 0.3$, $m = 5$, $B = 50$.
\modified{The number of active and noise features is held fixed at 7 and 3, respectively; additional features beyond $K = 20$ are null ($\alpha_k = \beta_k = \gamma_k = 0$).}
The coefficient $c$ and intercept $\beta_0$ are recalibrated for each SNR.

Tables~\ref{tab:middim} and \ref{tab:highdim} report F1 and AUC \modified{for DGP~1}.
\modified{PRIMED maintains a consistent AUC advantage across dimensionalities, though its F1 advantage is not strictly monotonic across $K$.}
...
\modified{Tables~\ref{tab:middim_dgp2} and \ref{tab:highdim_dgp2} report the corresponding results for DGP~2 ($\gamma = 0$).
...
\modified{In addition to F1 and AUC, we assess variable selection stability via the Jaccard index, averaged over all $\binom{B}{2}$ pairs of replications.
...
\modified{Figure~\ref{fig:scatter_cd} visualizes the Pearson correlation between $\bar X_k$ and $S_k$ for each ordinal feature.}
...
\modified{Inspection of Figure~\ref{fig:scatter_cd} reveals that several features exhibit nonlinear patterns, in particular inverted-U shapes where disagreement peaks at intermediate consensus values and drops near the scale boundaries.
...
\modified{Table~\ref{tab:app_coefs} summarizes the estimated coefficients; the corresponding fitted models are:}
\begin{align*}
...
\modified{To verify that the consensus--disagreement relationship for the selected features is genuine, we fit separate OLS regressions of $S_k$ on $\bar X_k$ for each selected feature.
...
\modified{The selection of Pre\_DWI is revisited in the sensitivity analysis below, where a quadratic dispersion model reveals that its apparent disagreement signal is largely an artifact of linear misspecification.}

\modified{\subsubsection{Sensitivity analysis: quadratic dispersion model}\label{sec:quad_sensitivity}}

\modified{To quantify the nonlinear consensus--disagreement patterns, we compare linear and quadratic dispersion models for each ordinal feature.
...
\modified{Despite using only one feature, the quadratic PRIMED achieves an external AUC of $0.766$, essentially matching C~LASSO ($0.767$ with three variables) and narrowing the gap with CD~GL ($0.782$).
...
%%-------------------------------------------------------------------
\modified{\subsection{Application: LIDC-IDRI lung nodule radiomics}\label{sec:lidc}}
%%-------------------------------------------------------------------

\modified{\subsubsection{Data and design}}

\modified{To validate PRIMED in a high-dimensional radiomics setting, we apply it to the Lung Image Database Consortium and Image Database Resource Initiative (LIDC-IDRI) dataset \cite{Armato2011}.
...
\modified{Prediction performance was evaluated via nested cross-validation: a stratified 10-fold outer CV assessed generalization, while within each outer training fold, a stratified 5-fold inner CV selected $\lambda$ (using the same data-driven path and deviance criterion described in Section~\ref{sec:lambda_cv}).
...
\modified{\subsubsection{Prediction and variable selection}}

\modified{Table~\ref{tab:lidc_auc} reports the 10-fold CV AUC and the number of features selected.
...
\begin{table}[h]
\caption{\modified{LIDC-IDRI lung nodule radiomics: 10-fold CV performance}}\label{tab:lidc_auc}
\small
\renewcommand{\arraystretch}{1.1}
\begin{tabular*}{\textwidth}{@{\extracolsep\fill}lcc}
\toprule
\modified{Method} & \modified{AUC (SD)} & \modified{Features selected} \\
\midrule
\modified{PRIMED}  & \modified{$0.900$ ($0.035$)} & \modified{$65.6$} \\
\modified{CD~GL}   & \modified{$0.905$ ($0.033$)} & \modified{$30.7$} \\
\modified{C~LASSO} & \modified{$0.898$ ($0.034$)} & \modified{$23.2$} \\
\botrule
\end{tabular*}
\footnotetext{\modified{Mean (SD) over 10 stratified CV folds with nested 5-fold inner CV for $\lambda$ selection.}}
\end{table}

\modified{\subsubsection{Variable selection stability}}

\modified{Beyond point prediction, we assess the consistency of variable selection across CV folds using the Jaccard index \cite{Jaccard1912}: for two sets of selected features $A$ and $B$, $J(A, B) = |A \cap B| / |A \cup B|$.
...
\begin{table}[h]
\caption{\modified{LIDC-IDRI: variable selection stability (Jaccard index)}}\label{tab:lidc_jaccard}
\small
\renewcommand{\arraystretch}{1.1}
\begin{tabular*}{\textwidth}{@{\extracolsep\fill}lcc}
\toprule
\modified{Method} & \modified{Jaccard (SD)} & \modified{Features selected} \\
\midrule
\modified{PRIMED}  & \modified{$\mathbf{0.798}$ ($0.053$)} & \modified{$65.6$} \\
\modified{CD~GL}   & \modified{$0.688$ ($0.053$)} & \modified{$30.7$} \\
\modified{C~LASSO} & \modified{$0.560$ ($0.126$)} & \modified{$23.2$} \\
\botrule
\end{tabular*}
\footnotetext{\modified{Jaccard index averaged over all $\binom{10}{2} = 45$ pairs of CV folds.}}
\end{table}
...
\modified{PRIMED's advantage stems from three design choices that work in concert.
...
\modified{We proposed PRIMED, a joint loss framework for multi-reader imaging data that integrates consensus, disagreement, and their structural relationship into a single penalized objective.
...
\end{axis}
\end{tikzpicture}
\caption{\modified{F1 score as a function of subject-specific noise $\tau$.
...
\end{axis}
\end{tikzpicture}
\caption{\modified{F1 score as a function of dimensionality $K$.
Results shown for S2 (noise present) under DGP~1 (top) and DGP~2 (bottom).
...
\modified{Red curves indicate the fitted quadratic regression $S_k = \alpha_{0k} + \alpha_{1k} \bar X_k + \alpha_{2k} \bar X_k^2 + \varepsilon_k$; shaded bands show 95\% confidence intervals.
...
\For{$t = 1, 2, \dots, T_{\max}$}
  ...
  \For{$k = 1, \dots, K$}
    \If{$\|(\tilde\beta_k, \tilde\gamma_k)\|_2 > \eta\lambda$}
      \State $(\beta_k, \gamma_k) \gets \bigl(1 - \tfrac{\eta\lambda}{\|(\tilde\beta_k, \tilde\gamma_k)\|_2}\bigr)(\tilde\beta_k, \tilde\gamma_k)$
    \Else
...
\modified{LIDC-IDRI: Lung Image Database Consortium and Image Database Resource Initiative;
HU: Hounsfield units;
...
This study was approved by the institutional review board of each participating center. Informed consent was obtained from all participants.

\subsection*{Consent for publication}
Not applicable.

\subsection*{Availability of data and materials}
The LIDC-IDRI dataset analysed during the current study is publicly available in The Cancer Imaging Archive (TCIA) repository, https://doi.org/10.7937/K9/TCIA.2015.LO9QL9SX \cite{Armato2011}. The rectal cancer MRI dataset used during the current study is available from the corresponding author on reasonable request.
...
\modified{When nonlinear patterns are present, the quadratic extension $S_k = \alpha_{0k} + \alpha_{1k} \bar X_k + \alpha_{2k} \bar X_k^2 + \varepsilon_k$ (Section~\ref{sec:nonlinear}) can substantially improve the explained variance; e.g., from $R^2 = 0.085$ to $0.695$ for Fibrosis in the rectal cancer data (Table~\ref{tab:disp_r2}).
...
\modified{This is the setting used in the LIDC-IDRI radiomics application (Section~\ref{sec:lidc}), where each feature is a continuous quantity (e.g., volume in mm$^3$, entropy in nats) computed independently from each reader's segmentation mask.
...
\modified{In the rectal cancer application (Section~\ref{sec:application}), all 10 ordinal MRI features (scored on 3--6 point scales by five radiologists) are summarized using the mean and SD, following this continuous approximation.}
...
Table~\ref{tab:ordinal_summary} summarizes the available measures by scale length. \modified{Both applications in this paper use the mean and SD for all ordinal features.}
...
3--4 categories & Mean or Median & SD, Leik's $D$, $1-l^2$ & \modified{Mean/SD used in this paper; ordinal-specific alternatives available} \\
\botrule
\end{tabular*}
\end{table}
...
This is the approach taken in \modified{the rectal cancer application (Section~\ref{sec:application})}, where four binary MRI features enter the model as reader-averaged proportions.
\modified{The LIDC-IDRI application does not involve binary features, as all 107 radiomics features are continuous.}
...
\toprule
Scale & Consensus & Dispersion & PRIMED penalty & \modified{Application} \\
\midrule
Continuous & Mean & SD & Group $(\beta_k, \gamma_k)$ & \modified{LIDC-IDRI} \\
Ordinal & Mean & SD & Group $(\beta_k, \gamma_k)$ & \modified{Rectal} \\
Binary & Mean ($= p$) & Not applicable & LASSO ($\delta_k$) & \modified{Rectal} \\
